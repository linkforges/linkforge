...
<form method="POST" action="{{ route('admin.settings.update') }}" class="mt-6 space-y-6">
    ...
    <div class="lf-card border border-slate-200 bg-slate-50 p-5 text-xs text-slate-500">
        Geo resolution prefers the IP2Location HTTP API when API keys are configured.
        Otherwise it falls back to a local GeoIP database (DB-IP / GeoLite2) for country
        lookups.
    </div>

    <div class="lf-card border border-slate-200 bg-slate-50 p-5 text-xs text-slate-500">
        Add one or more IP2Location API keys in your .env via <code>IP2LOCATION_API_KEYS</code>.
        The app rotates keys randomly on each lookup to distribute usage.
    </div>
