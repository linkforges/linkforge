{{-- "Continue with Google" button. Renders only when Google login is configured + enabled. --}}
@if (config('services.google.enabled') && config('services.google.client_id'))
    <a href="{{ route('auth.google.redirect') }}"
       class="flex w-full items-center justify-center gap-2.5 rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
